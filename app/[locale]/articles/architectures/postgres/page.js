...
export default async function PostgreSQLArchitecture() {
  ...
  const DiagramNode = ({ children, className, label }) => (
    <div className={`relative ${className}`}>
      <div className="rounded-lg px-4 py-3 border-2 shadow-md text-center !bg-white dark:!bg-slate-700 border-slate-300 dark:border-slate-600">
        {children}
      </div>
      {label && (
        <div className="absolute -bottom-5 left-1/2 transform -translate-x-1/2 text-xs text-slate-600 dark:text-slate-300 whitespace-nowrap">
          {label}
        </div>
      )}
    </div>
  ...
  const Arrow = ({ direction = "down", label }) => (
    <div className="flex flex-col items-center justify-center py-2">
      <MdArrowDownward
        className={`text-cyan-600 dark:text-cyan-400 ${direction === 'up' ? 'rotate-180' : ''}`}
        size={24}
      />
      {label && <span className="text-xs dark:text-slate-200 mt-1 font-medium">{label}</span>}
    </div>
  );
      ...
      <article className="max-w-5xl mx-auto px-4 py-8">
        ...
        <header className="mb-8">
          <div className="flex items-center gap-3 mb-3">
            <MdStorage className="text-cyan-600 dark:text-cyan-400" size={40} />
          ...
          <div className="flex gap-3 mt-4 text-sm text-slate-600 dark:text-slate-300">
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-center">
              <DiagramNode className="w-48" label="">
                ...
                <span className="font-semibold text-slate-900 dark:text-white">{t('diagram.clientApplication')}</span>
              </DiagramNode>
            </div>
            ...
            <div className="flex justify-center">
              <DiagramNode className="w-56" label="">
                ...
                <span className="font-semibold text-slate-900 dark:text-white">{t('diagram.postmaster')}</span>
                <div className="text-xs text-slate-600 dark:text-slate-300 mt-1">{t('diagram.mainSupervisor')}</div>
              </DiagramNode>
            </div>
            ...
            <div className="flex justify-center">
              <DiagramNode className="w-52" label="">
                ...
                <span className="font-semibold text-slate-900 dark:text-white">{t('diagram.backendProcess')}</span>
                <div className="text-xs text-slate-600 dark:text-slate-300 mt-1">{t('diagram.onePerConnection')}</div>
              </DiagramNode>
            </div>
            ...
            <div className="bg-amber-100 dark:bg-amber-900/30 rounded-lg p-4 border-2 border-amber-500">
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <div className="bg-cyan-200 dark:bg-cyan-800 rounded p-2 text-xs text-center text-cyan-900 dark:text-cyan-100">
        ...
        <div className="prose prose-slate dark:prose-invert max-w-none">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {['backgroundWriter', 'checkpointer', 'walWriter', 'autovacuum'].map(worker => (
                  <div key={worker} className="border-l-4 border-yellow-500 dark:border-yellow-400 pl-4">
                    <h4 className="font-semibold text-slate-900 dark:text-white mb-1">{t(`processArchitecture.workers.${worker}.title`)}</h4>
                    <p className="text-sm text-slate-700 dark:text-slate-200">{t(`processArchitecture.workers.${worker}.description`)}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-lg p-6 shadow-md border border-slate-200 dark:border-slate-700 mb-6">
              ...
              <div className="grid md:grid-cols-3 gap-4 mb-4">
                <div className="bg-amber-50 dark:bg-amber-900/20 rounded p-3">
                  ...
                  <div className="text-sm text-slate-800 dark:text-slate-200">{t('memoryArchitecture.sharedBuffers.sizeValue')}</div>
                </div>
                <div className="bg-amber-50 dark:bg-amber-900/20 rounded p-3">
                  <div className="font-semibold text-amber-700 dark:text-amber-300 mb-1">{t('memoryArchitecture.sharedBuffers.pageSize')}</div>
                  <div className="text-sm text-slate-800 dark:text-slate-200">{t('memoryArchitecture.sharedBuffers.pageSizeValue')}</div>
                </div>
                <div className="bg-amber-50 dark:bg-amber-900/20 rounded p-3">
                  <div className="font-semibold text-amber-700 dark:text-amber-300 mb-1">{t('memoryArchitecture.sharedBuffers.goal')}</div>
                  <div className="text-sm text-slate-800 dark:text-slate-200">{t('memoryArchitecture.sharedBuffers.goalValue')}</div>
                </div>
              </div>
              <div className="bg-blue-50 dark:bg-blue-900/20 rounded p-4 border-l-4 border-blue-500 dark:border-blue-400">
                <p className="text-sm text-slate-800 dark:text-slate-200">
                  <strong>{t('memoryArchitecture.sharedBuffers.configuration')}</strong> <code className="bg-slate-200 dark:bg-slate-700 px-2 py-1 rounded text-slate-900 dark:text-slate-100">{t('memoryArchitecture.sharedBuffers.configValue')}</code> in postgresql.conf
                </p>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-lg p-6 shadow-md border border-slate-200 dark:border-slate-700 mb-6">
              ...
              <div className="space-y-4">
                <div className="border-l-4 border-amber-500 dark:border-amber-400 pl-4">
                  <h4 className="font-semibold text-slate-900 dark:text-white mb-2">{t('memoryArchitecture.localMemory.workMem.title')}</h4>
                  <p className="text-sm text-slate-700 dark:text-slate-200 mb-2">
                    {t('memoryArchitecture.localMemory.workMem.description')}
                  </p>
                  <div className="bg-amber-50 dark:bg-amber-900/20 rounded p-3 text-sm text-slate-800 dark:text-slate-200">
                    <strong>{t('memoryArchitecture.localMemory.workMem.important')}</strong> {t('memoryArchitecture.localMemory.workMem.importantText')}
                  </div>
                </div>
                <div className="border-l-4 border-amber-500 dark:border-amber-400 pl-4">
                  <h4 className="font-semibold text-slate-900 dark:text-white mb-2">{t('memoryArchitecture.localMemory.maintenanceWorkMem.title')}</h4>
          ...
          <section className="mb-12">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-lg p-6 shadow-md border border-slate-200 dark:border-slate-700">
              ...
              <div className="bg-slate-50 dark:bg-slate-800 rounded p-4 border border-slate-300 dark:border-slate-600">
          ...
          <section className="mb-12">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-lg p-6 shadow-md border border-slate-200 dark:border-slate-700">
              ...
              <div className="grid md:grid-cols-3 gap-4">
                <div>
                  <h4 className="font-semibold text-slate-900 dark:text-white mb-2">{t('mvcc.howItWorks')}</h4>
                ...
                <div>
                  <h4 className="font-semibold text-slate-900 dark:text-white mb-2">{t('mvcc.benefits')}</h4>
                  <ul className="text-sm text-slate-700 dark:text-slate-200 space-y-1">
                ...
                <div>
                  <h4 className="font-semibold text-slate-900 dark:text-white mb-2">{t('mvcc.tradeoffs')}</h4>
                  <ul className="text-sm text-slate-700 dark:text-slate-200 space-y-1">
          ...
          <section className="mb-8">
            <div className="bg-gradient-to-r from-cyan-50 to-purple-50 dark:from-cyan-900/20 dark:to-purple-900/20 rounded-lg p-6 border-2 border-cyan-300 dark:border-cyan-700">
              <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-4">{t('keyTakeaways.title')}</h2>
              <div className="grid md:grid-cols-2 gap-4 text-sm">
                {t.raw('keyTakeaways.items').map((item, i) => (
                  <div key={i} className="flex items-start gap-2">
                    <MdCheckCircle className="text-cyan-600 dark:text-cyan-400 mt-0.5 flex-shrink-0" size={20} />
